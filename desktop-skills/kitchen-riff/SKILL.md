...
A riff can just end as understanding — don't force a recipe. But when it lands on a keeper, offer to capture it: structure it as a recipe object and follow the **recipe-save contract** (`_recipe-saver.md`, bundled with this skill) — choosing `kind` (a finished dish is `standard`, a loose regular `quick-staple`, a riff on a collection recipe `variant`). For a full deep-dive on a dish, that's the `research-recipe` skill instead. Use the house naming convention and confirm before writing anything to the repo.
