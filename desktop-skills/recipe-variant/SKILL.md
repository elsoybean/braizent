...
Assemble the variant as a structured object with `kind: variant`, `base_slug` (the original's slug) and `suffix` (e.g. `vegetarian`, `double`, `precise`), plus the ingredients and steps. (If the variant is itself a loose quick staple, use `kind: quick-staple` instead.) Follow the **recipe-save contract** (`_recipe-saver.md`, bundled in this skill folder): it writes the variant beside the original, adds its `.cook` (variants get one; quick-staples don't), links back to the original, won't overwrite, and commits. Don't re-derive the paths or format.
